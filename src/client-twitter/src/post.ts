...
Think step-by-step before generating a post.

Please follow these steps in your chain-of-thought:
1. Identify the key elements or theme you have in mind and tie it to the references from the context you have from your knowledge base, recent messages, and other information you have.
2. Break down the concept into smaller logical steps, thinking logically about how to incorporate all the information you have at your disposal.
...
5. Use the above reasoning to generate your final, well-structured technical post.

# FORMAT: Please format your post using the following structure:
...
Your response (the final answer, separate from your chain-of-thought) should be 1, 2, or 3 sentences (choose the length at random).
Your response should not contain any questions. Brief, concise statements only. The total character count (for the final response) MUST be less than {{maxTweetLength}}. No emojis. Use \\n\\n (double spaces) between statements if there are multiple statements in your response.`;
...
Before selecting an action, follow this step-by-step **chain-of-thought** process:

1. **Context Check** – Analyze the tweet's content:
   - Does it mention a **new project, token, or presale** relevant to AlphaAgent's focus?
   - Is there a **trading signal** such as a **notable trader follow, wallet movement, or liquidity injection**?
   - Does it align with **crypto market intelligence** (e.g., alpha leaks, early-stage opportunities)?

2. **Alpha Signal Analysis** – Assess if the tweet contains **meaningful insights**:
   - Does it originate from a **high-value trader** or reputable source?
   - Is there **on-chain data to support** the claim?
   - Are **top traders engaging** with it, or is it mostly bot activity?

3. **Relevance Filter** – Determine if the content is **high-signal or noise**:
   - If **off-topic, engagement farming, or purely promotional**, **IGNORE**.
   - If **directly related to trading intelligence, presale opportunities, or blockchain data**, **consider engaging**.

4. **Action Selection** – Choose the **appropriate action based on tweet quality**:
   - **LIKE** → **Perfect alignment** with AlphaAgent’s expertise (9.8/10 relevance).
   - **RETWEET** → **Exceptional** alpha insight that others should see (9.5/10 relevance).
   - **QUOTE** → AlphaAgent can add **substantial domain expertise** (9.5/10 relevance).
   - **REPLY** → AlphaAgent can contribute **expert-level insight** (9.5/10 relevance).
   - **NO ACTION** → If relevance is low or engagement adds no strategic value.
...
<chain-of-thought>
1. **Context Check:** The tweet is from a reputable trader discussing a **new Solana project**.
2. **Alpha Signal Analysis:** The project just received **liquidity injections**, and **notable traders** have followed it.
3. **Relevance Filter:** This aligns **directly** with AlphaAgent’s focus—tracking **early trading signals**.
4. **Action Selection:** The tweet is **high-value intelligence** but does not require additional commentary.
</chain-of-thought>

Final Action: [LIKE]

<chain-of-thought>
1. **Context Check:** The tweet provides **on-chain evidence** of a whale accumulating a new token.
2. **Alpha Signal Analysis:** Liquidity injections match **recent influencer follows**, suggesting a **possible early trade setup**.
3. **Relevance Filter:** Other traders are **starting to take notice**, but it's still under the radar.
4. **Action Selection:** Retweeting **amplifies** this alpha without needing additional commentary.
</chain-of-thought>

Final Action: [RETWEET]

<chain-of-thought>
1. **Context Check:** The tweet is from an **anonymous account** promoting a low-cap token.
2. **Alpha Signal Analysis:** No **wallet movements** or **on-chain activity** to validate the claim.
3. **Relevance Filter:** The post has **low engagement** (mostly bots and giveaway hunters).
4. **Action Selection:** No actionable trading insight—engagement **adds no value**.
</chain-of-thought>
...
# **Final Decision**
Respond ONLY with the most **qualifying action tags**. Default to **NO ACTION** unless **extremely confident of relevance**.

<chain-of-thought>
(AlphaAgent's reasoning process goes here, incorporating market intelligence, blockchain signals, and trading insights)
</chain-of-thought>
...
export class TwitterPostClient {
  ...
  async start() {
    ...
    const generateNewTweetLoop = async () => {
      ...
      elizaLogger.info(`Last post timestamp: ${new Date(lastPostTimestamp).toISOString()}`);
  ...
  async generateNewTweet() {
    ...
    try {
      ...
      elizaLogger.info('Generated tweet:', JSON.stringify(alteredResponse, null, 2));
      const finalAnswerMarker = 'Final Answer:';
      let chainOfThoughtText = '';
      let finalAnswer = '';

      if (alteredResponse.includes(finalAnswerMarker)) {
        const parts = alteredResponse.split(finalAnswerMarker);
      ...
      const rawTweetContent = cleanJsonResponse(finalAnswer);
  ...
  private async generateTweetContent(
    ...
    const cleanedResponse = cleanJsonResponse(finalAnswer);
      ...
      try {
        ...
        if (actionResponse.quote) {
          try {
            ...
            const quoteContent = await this.generateTweetContent(roomId, enrichedState, {
              template:
        ...
        if (actionResponse.reply) {
          try {
            await this.handleTextOnlyReply(roomId, tweet, tweetState, executedActions);
          } catch (error) {
  ...
  private async handleTextOnlyReply(
    ...
    try {
      ...
      const replyText = await this.generateTweetContent(roomId, enrichedState, {
        template:
          this.runtime.character.templates?.twitterMessageHandlerTemplate ||
          twitterMessageHandlerTemplate,
      });
